/*
 * loader.c: slightly modified from E9Patch (https://github.com/GJDuck/e9patch)
 */
...
static NO_INLINE int loader_get_binary(char *path_buf);
...
static int loader_open(const char *filename_0, int flags_0, int mode_0) {
...
static int loader_write(int fd_0, const char *buf_0, size_t len_0) {
...
static int loader_kill(pid_t pid_0, int sig_0) {
...
static int loader_readlink(const char *path_0, char *buf_0, size_t len_0) {
...
static int loader_getdents64(int fd_0, struct linux_dirent64 *dirp_0,
                             unsigned count_0) {
    register uintptr_t fd asm("rdi") = (uintptr_t)fd_0;
...
static int loader_close(int fd_0) {
    register uintptr_t fd asm("rdi") = (uintptr_t)fd_0;
    register intptr_t err asm("rax");
 ...
 */
static char *loader_num2str(char *str, int x) {
    int r = 1000000000;
    bool seen = false;
    while (r != 0) {
        char c = '0' + x / r;
 ...
 */
NO_INLINE NO_RETURN void loader_error(const char *err_str, int err) {
    char path_buf[BUFSIZ];
    int result = loader_get_binary(path_buf);
    if (result != 0) {
    ...
    while (*err_str_0 != '\0') *str++ = *err_str_0++;
    while (*err_str != '\0') {
        if (*err_str == '%') {
            err_str++;
            switch (*err_str++) {
                case 'd':
                    str = loader_num2str(str, err);
                    break;
                case 's':
                    while (*path != '\0') *str++ = *path++;
                    break;
            }
        } else
            *str++ = *err_str++;
    }
    (void)loader_write(STDERR_FILENO, str_buf, str - str_buf);
    (void)loader_kill(/*pid=*/0, SIGABRT);
    asm volatile("ud2");
 ...
 */
static intptr_t loader_hexstr2num(const char **str_ptr) {
...
static NO_INLINE int loader_get_binary(char *path_buf) {
    ...
    int fd = loader_open(mapsname, O_RDONLY | O_DIRECTORY, 0);
    if (fd < 0) return fd;
    uint8_t buf[BUFSIZ];
    int len = loader_getdents64(fd, (struct linux_dirent64 *)buf, BUFSIZ);
    loader_close(fd);
    if (len < 0) return len;

    // Step (2): Search for an entry corresponding to the enclosing function.
    intptr_t code = (intptr_t) & loader_get_binary;
    bool found = false;
    for (int i = 0; !found && i < len;) {
        ...
        intptr_t lo = loader_hexstr2num(&name);
        if (code < lo) continue;
        intptr_t hi = loader_hexstr2num(&name);
        if (code <= hi) {
            int j;
            for (j = 0; mapsname[j] != '\0'; j++) path_buf[j] = mapsname[j];
            name = entry->d_name;
            for (int k = 0; name[k] != '\0'; k++) path_buf[j++] = name[k];
            path_buf[j] = '\0';
            found = true;
        }
    }
    ...
    //       opened directy.
    len = loader_readlink(path_buf, path_buf, BUFSIZ);
    if (len < 0) return len;
    path_buf[len] = '\0';
    return 0;
}
...
int loader_entry(void) {
    char path_buf[BUFSIZ];
    int err = loader_get_binary(path_buf);
    if (err != 0) loader_error(maps_err_str, -err);

    int fd = loader_open(path_buf, O_RDONLY, 0);
    if (fd < 0) loader_error(open_err_str, -fd);
    return fd;
}
